from scriptparser import scene,style_type, side
import ffmpeg
import ass,re
...
def compose_scenes(script: tuple[dict,list[scene]]):
    ...
    for scene in script_body:
        ...
        if scene.style.style == style_type.PIP:
            ...
            # cut the audio out
            ffmpeg.output(scene.avatar_video.video.audio, f"{scene.avatar_video.filename}.mp3").run(overwrite_output=True)
            ...
            if scene.style.avatar_position == side.RIGHT:
                ...
                #ffmpeg.concat(out, clip.audio, v=1, a=1).output("AH.mp4").run()  overwrite_output=True
            ...
            else: # default to left processing
                margin = 10
                c = 4*margin
                output_dim = scene.style.output_dim
                ...
                bkgnd_slides = ffmpeg.filter([bckgrnd, scaled_slides], 'overlay', shortest=1, x=str(output_dim[0]-margin-scaled_slides_dim[0]), y='(main_h-overlay_h)/2') #*['10','(main_h-overlay_h)/2'])
                out = ffmpeg.filter([bkgnd_slides, cropped_avatar], 'overlay', shortest= 1, x=str(margin), y='(main_h-overlay_h)/2')
